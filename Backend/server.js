...
if (!process.env.JWT_SECRET) {
  if (process.env.NODE_ENV === "production") {
    ...
    console.warn(
      "JWT_SECRET not set, using development secret. DO NOT USE IN PRODUCTION!"
    );
...
const MONGODB_URI =
  ...
  "mongodb+srv://nikeshvaishnav377:Inda9636@cluster0.mahsunm.mongodb.net/";
mongoose
  .connect(MONGODB_URI)
  .then(async () => {
    ...
    const server = app.listen(PORT, HOST, () => {
      ...
      if (address) {
        ...
        console.warn(
          "⚠️ Could not retrieve server address details (may be undefined on Render)."
        );
      }
    });
